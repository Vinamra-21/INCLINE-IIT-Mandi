...
interface EcoPulseItem {
  ...
  Url: string;
  Category: string;
  Type: "news" | "journal"; // Added Type to distinguish between news and journals
}
...
interface FeedSource {
  ...
  url: string;
  category: string;
  type: "news" | "journal"; // Added type to categorize sources
}
...
const EcoPulse: React.FC = () => {
  ...
  const feedSources: FeedSource[] = [
    ...
    },
    {
      ...
      url: "https://climate.nasa.gov/feed/",
      category: "Climate Tech",
      type: "news",
    },
    ...
    {
      name: "Environmental Science & Technology",
      url: "https://pubs.acs.org/journal/esthag/0/0", // Using a different format since actual RSS unavailable
      category: "Environmental Science",
      type: "journal",
    },
  ];
  ...
  const sections = ["All", "News", "Journals"];
          ...
          try {
            ...
            const transformedItems: EcoPulseItem[] = newsItems.map(
              (item: any, index: number) => {
                ...
                // Extract image only for news type (not for journals)
                let imageUrl = "";
                if (source.type === "news") {
                  // Try to get image from enclosure
                  if (item.enclosure && item.enclosure["@_url"]) {
                ...
                return {
                  ...
                  Url: link,
                  Category: source.category,
                  Type: source.type, // Set the type based on the source
                };
              }
            );
  ...
  const hasRealImage = (item) => {
    return item.Image && item.Image.trim() !== "" && !failedImages[item.id];
  };
  ...
  const filteredItems = items.filter((item) => {
    const matchesSection =
      activeSection === "All" ||
      (activeSection === "News" && item.Type === "news") ||
      (activeSection === "Journals" && item.Type === "journal");
  ...
  const newsItems = filteredItems
    .filter((item) => item.Type === "news")
    .sort((a, b) => {
      // First sort by whether they have a real image
      ...
      // If both have images or both don't have images, sort by date
      return new Date(b.Date).getTime() - new Date(a.Date).getTime();
    });
  const journalItems = filteredItems
    ...
    <div className="transition-colors duration-300 min-h-screen p-6 bg-gradient-to-b from-green-50 to-blue-50 dark:bg-gradient-to-b dark:from-gray-900 dark:to-gray-800 text-gray-900 dark:text-gray-100">
      <div className="mt-16 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-300 text-center">
            Your professional gateway to global environmental trends and water
        ...
        {!isLoading && (
          <div className="space-y-12">
            ...
            {/* Journals Section */}
            {(activeSection === "All" || activeSection === "Journals") &&
                ...
                <div>
                  <div className="grid grid-cols-1 gap-4">
                    {journalItems.map((item) => (
                      <div
                        key={item.id}
                        className="rounded-lg shadow-md hover:shadow-xl transition-all duration-300 bg-white dark:bg-gray-800 border-l-4 border-blue-500 p-6">
                        <div className="flex flex-col">
                          <div className="mb-4">
                            ...
                            <div className="flex items-center mt-2 text-sm text-gray-500 dark:text-gray-400">
                              ...
                              <span className="ml-3 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-2 py-1 rounded-full text-xs">
                                {item.Category}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="mt-auto text-right">
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="bg-blue-600 text-white px-4 py-2 rounded-md text-sm transition duration-300 ease-in-out hover:bg-blue-700 inline-block">
                              View Publication
                            </a>
                          </div>
                        </div>
            ...
            {!isLoading && filteredItems.length === 0 && (
              <div className="text-center p-12 bg-white dark:bg-gray-800 rounded-lg shadow">
                <p className="text-xl">No matching articles found.</p>
                <p className="mt-2 text-gray-500">
                  Try adjusting your search or section filters.
                </p>
              </div>
            )}
          </div>
